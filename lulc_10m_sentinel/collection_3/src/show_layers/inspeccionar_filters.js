/**
 * @title App de Visualização de Filtros Sentinel-2 (Caatinga) - v4 (Estável)
 * @description Visualizador sem rótulos e com Coleção 10 ativa por padrão.
 * @author Expert GEE
 */
...
var assets = {
    filters: {
        ...
        'To Export': 'projects/mapbiomas-brazil/assets/LAND-COVER-10M/COLLECTION-3/GENERAL/CAATINGA/POS-CLASS/to_export',
    },
...
var availableVersions = [ '4', '5', '6']; // '1', '2', '3',

// Lista de nomes/IDs para as bacias (usada apenas no Dropdown agora)
var listaNameBacias = [
...
// Estilos compatíveis com GEE
var styles = {
    panel: {
        width: '350px', 
        padding: '10px', 
        backgroundColor: '#ffffff',
        border: '1px solid #cccccc' // Estilo corrigido
    },
    card: {
...
function toggleSidebar() {
    ...
    if (isShown) {
        ...
        showButton.style().set('shown', true);    
    } else {
        controlPanel.style().set('shown', true);  
        showButton.style().set('shown', false);   
    }
}
...
function updateMaps() {
    ...
    // Coleção 10 (MapBiomas) - AGORA TRUE (ATIVO POR PADRÃO)
    var mapbiomas = ee.Image(assets.uso_cobertura)
        .select(bandName)
        .updateMask(mask_active);
    leftMap.addLayer(mapbiomas, vis.map_class, 'Coleção 10 (LULC)', true);
...
print("App v4 carregado com sucesso.");