...
param = {
    ...
    # 'output_asset': 'projects/mapbiomas-workspace/AMOSTRAS/col10/CAATINGA/S2/POS-CLASS/Spatials_all',
    'versionOut' : 5,
    'versionInp' : 5,
    'last_year' : 2025,
    'first_year': 2016,
...
def apply_spatialFilterConn(name_bacia):
    ...
    for nyear in lstyear:
        ...
        rasterSpatialYear = rasterSpatialYear.where(raster_afloramento.gt(0), raster_afloramento)
        ...
        if name_bacia in list_bacia_flo:
            ...
            is_savana = rasterSpatialYear.eq(4)
            is_ref_forest = cobertYY.eq(3)
            is_inside_poly = mask_florest.eq(1)
            
            # Combina as condições
            condition_fix = is_savana.And(is_ref_forest).And(is_inside_poly)
            
            # Aplica a substituição: Onde a condição for verdadeira, vira 3 (Floresta)
            rasterSpatialYear = rasterSpatialYear.where(condition_fix, 3)
            # sys.exit()
    ...
    name_exp = f"filterGF_BACIA_{name_bacia}_GTB_V{param['versionOut']}"
