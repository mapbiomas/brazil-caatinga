...
def apply_spatialFilterConn(name_bacia):
    ...
    for nyear in lstyear:
        ...
        if nyear < 2025:
            cobertYY = mapsCobertura.updateMask(bacia_raster).select(banda_activa)
        else:
            cobertYY = mapsCobertura.updateMask(bacia_raster).select('classification_2024')

        ## removendo as confussões de floresta 
        regra = rasterSpatialYear.eq(3).And(cobertYY.neq(3))
