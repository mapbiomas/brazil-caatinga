...
class ClassMosaic_indexs_Spectral(object):
    ...
    options = {
        ...
        'classNew':  [3, 4, 3, 3, 12, 12, 15, 18, 18, 18, 18, 25, 25, 25, 25, 33, 29, 25, 33, 12, 33, 18, 18, 18, 18, 18, 18, 18,  4,  4, 21],
        'classMapBS2': [3, 4, 5, 9, 11, 12, 13, 15, 18, 19, 20, 21, 22, 23, 24, 25, 26, 30, 31, 32, 33, 36, 39, 40, 41, 46, 47, 62],
        'classNewS2':  [3, 4, 3, 3, 12, 12, 12, 15, 15, 15, 15, 21, 25, 25, 25, 25, 33, 33, 33, 33, 33, 15, 15, 15, 15, 15, 15, 15],

        'asset_bacias_buffer' : 'projects/mapbiomas-workspace/AMOSTRAS/col9/CAATINGA/bacias_buffer_caatinga_49_regions',
        ...
        'assetclassS2' : 'projects/mapbiomas-workspace/AMOSTRAS/col9/CAATINGA/S2/Classifier/ClassVX',
        ...
        'asset_output': 'projects/nexgenmap/SAMPLES/Caatinga/ROIs',
        'lsClasse': [4, 3, 12, 15, 18, 21, 25, 33],
        'lsPtos': [1000, 2000, 1000, 1500, 1500, 1000, 1500, 1000],
        "anoIntInit": 2016,
        "anoIntFin": 2023,
    }
    ...
    def __init__(self):
        ...
        # print("dict statistics ", self.dict_Statgeral['2016'] )
        # print(self.dict_Statgeral.keys())
        # sys.exit()
        ...
        collection90 = ee.Image(self.options['assetMapbiomas90']).updateMask(raster_bacias)
        # collectionS2 = ee.ImageCollection(self.options['assetclassS2']).min()
        # # Remap todas as imagens mapbiomas
        lsBndMapBiomnas = []
        self.imgMapstSample = ee.Image().toByte()
        # self.imgMapbiomasS2 = ee.Image().toByte()
        for year in self.lst_year:
            ...
            # imgTempAgro =  imgTemp.eq(18).multiply(18)
            # imgtmpS2 = collectionS2.select(band).remap(
            #                     self.options['classMapBS2'], self.options['classNewS2'])
            # imgtmpS2 = imgtmpS2.where(imgTempAgro.eq(18), imgTempAgro)
            self.imgMapstSample = self.imgMapstSample.addBands(
                                                        imgTemp.rename(band))

        self.imgMapstSample = self.imgMapstSample.select(lsBndMapBiomnas)#.clip(self.regionInterest.geometry())
    ...
    def iterate_bacias(self, idGrade, askSize):        
        ...
        for nyear in self.lst_year[:]:
            ...
            simgMosaic = ee.ImageCollection(self.options['asset_mosaic_sentinel']
                                                                ...
                                                                ).filterBounds(oneGrade)#.select(arqParam.featureBands)
            ...
            for cc, nclass in enumerate(self.options['lsClasse'][:]):
                ...
                layerCC = self.imgMapstSample.select(bandYear).eq(int(nclass)).multiply(int(nclass)).rename('class')
            ...
            print(f"======  coleted rois from all class in Year {nyear}  =======")
...
def ask_byGrid_saved(dict_asset):
    ...
    for cc, idAsset in enumerate(getlstFeat[:]):         
        path_ = idAsset.get('id')        
        name_feat = path_.replace( assetbase + '/', '')
        print(f" #{cc} reading <==> " + name_feat)
...
if searchFeatSaved: 
    ...
    lstFeatAsset = []
gInic = 0  # 72 515, 83, 667, 160 708 , 197,203, 715, 203, 740, 245, 755 ,316, 425
gEnd = 1000   # 500, 1000
# sys.exit()
for cc, item in enumerate(lstIdCode[gInic: gEnd]):
    print(f"# {cc + 1 + gInic} loading geometry grade {item}")   
    if item not in lstFeatAsset:
        featSaved = objetoMosaic_exportROI.iterate_bacias(item, askingbySizeFC)  # 
        if featSaved:
            print("coleta feita ")
        else:
            print("========= ocurrio algum erro ==== ")
        cont = gerenciador(cont, param)
    # sys.exit()
