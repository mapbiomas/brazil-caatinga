...
class Classify_Mosaic_process(object):
    ...
    options = {
        ...
        'asset_stattistics_basin': 'projects/mapbiomas-workspace/AMOSTRAS/col9/CAATINGA/S2/statisticS2',
        ...
        'asset_rois': 'projects/mapbiomas-workspace/AMOSTRAS/col9/CAATINGA/S2/ROIs/coleta3',
        'asset_rois_red': 'projects/mapbiomas-workspace/AMOSTRAS/col9/CAATINGA/S2/ROIs/coleta3red',
    ...
    def __init__(self, saveROIsBalance):
        ...
        print("statisticas nos anos ==> ", self.dict_Statgeral.keys())
    ...
    def process_re_escalar_img (self, imgA):
        ...
        dictStatYY = self.dict_Statgeral[str(self.year_activo)]
    ...
    def iterate_bacias(self, idBacia, myModel):        
        ...
        geoBasin = (
            ee.FeatureCollection(self.options['asset_bacias_buffer'])
        ...
        imgeoBasin = (
            ee.ImageCollection(self.options['asset_region_img_buffer'] )
                    .filter(ee.Filter.eq('isBuffer', 1))
                    .filter(ee.Filter.eq('nunivotto4', idBacia)).first().gt(0)
        )
        # print("image base loaded ", imgeoBasin.bandNames().getInfo())
        # imgeoBasin = ee.Image(imgeoBasin.first())        
        ...
        for nyear in self.lst_year[:]:
            ...
            # print("size ", imgMosaic.size().getInfo())
            time.sleep(1)
            ...
            if self.options['anoInicial'] == nyear:
                ...
                if byYear:
                    self.processoExportar(imglsClasxanos.select(bandActiva), geoBasin.coordinates(), nomec + "_" +  '_prob')
...
param = {
    'asset_input': 'projects/mapbiomas-workspace/AMOSTRAS/col9/CAATINGA/S2/Classifier/ClassVY',
    'anoInicial': 2016,
    'anoFinal': 2022,
    'changeCount': False,
    'numeroTask': 6,
