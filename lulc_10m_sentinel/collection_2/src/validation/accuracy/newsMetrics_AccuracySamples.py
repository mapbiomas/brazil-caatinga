...
base_path = '/run/media/superuser/Almacen/mapbiomas/dadosCol9'
input_path_CSVs = '/run/media/superuser/Almacen/mapbiomas/dadosCol9/ptosAccColS2corr'
...
def calculing_Aggrements_AccGlobal_ModelVers(dfAggP, dfacctmp):
    ...
    def calculing_Aggrements_AccGlobal(row):  
        ...
        if df_tmp.shape[0] > 0:
            ...
            row["shift"] = shiftV
            name = base_path + '/conf_matrixS2/CM_' +  nbacia + "_" + model + "_" + str(yyear) +  "_" + str(vers) + '.csv' 
...
lstColRef = ['CLASS_' + str(kk) for kk in range(2016, 2023)]
lstColPred = ['classification_' + str(kk) for kk in range(2016, 2023)]
lYears = [kk for kk in range(2016, 2023)]
...
for nmodel in posclass[:]:
    for vers in version_process[:]:
        ...
        for cc, path in enumerate(lst_paths[:]): 
            ...
            print("numero de partes ", partes)
            version = partes[-2]
            posClass = None
            bacia = partes[3]
            if "GTB_" + vers in path:
                classificador = partes[4]
            else:
                classificador = partes[5] # posClass
                # bacia = partes[-4]
            print("classificador ==> ", classificador, " version ====> ", version)
            # sys.exit()
            if int(version) == 0:
            ...
            if str(nmodel) in str(classificador) and str(vers) == str(version): 
                ...
                df_CSV['version'] = [vers] * df_CSV.shape[0]
                df_CSV['models'] = [classificador] * df_CSV.shape[0]
                # add to list ofs Dataframes             
                lst_df.append(df_CSV)
        print(f"para o modelo {nmodel} and version {vers} we have {len(lst_df)}")
        # sys.exit()
        # if cc > 10:
        #     break
        if len(lst_df) >= 35:
            ...
            classInic = [3,4, 9,10,12,15,18,21,22,25,27,29,33,50]
            classFin  = [3,4,12,12,12,21,21,21,25,25,27,12,33, 3]
            ...
            if buildMetricsAcc: 
                ...
                pathOutpout = base_path + '/globalTables/'
                nameTablesGlob = f"regMetricsAccs_{nmodel}_vers_{vers}_ColS2.csv"   
            ...
            if buildMetAggrements:
                ...
                nameTablesGlob = f"regAggrementsAcc_{nmodel}_vers_{vers}_ColS2.csv"
        ...
        print("  =================================================================")
        print(f"    the model {nmodel} don´t have all basin , show here {48 - len(lst_df)}")
        for cc, dfBasin in enumerate(lst_df):
            print(f" #{cc} {dfBasin['bacia'].iloc[0]}")