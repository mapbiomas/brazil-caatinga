...
input_path_CSVs = '/run/media/superuser/Almacen/mapbiomas/dadosCol9/AREA-EXPORT-S2'
...
if processS2:
    ...
    for nmodel in posclass[:]:
        for vers in version_process:
            ...
            for pathLayerA in filesAreaCSV:
                ...
                partes = nameFiles.replace("areaXclasse_CAATINGA_ColS2", "")
                partes = partes.split("_")
                name_model = 'GTB' #partes[0]
                version = partes[-2]
                print("partes >>> ", partes)
                if len(partes) > 4:
                    name_model = partes[1]
            ...
            if len(lstDF) > 47:   
                ...
                print(f" === 📲 We have now <<{ndfArea.shape[0]}>> row in the {len(lstDF)} DataFrame Area ===")
                print(ndfArea.head())
                classInic = [ 0,3,4, 9,10,12,15,18,21,22,25,27,29,33,50]
                classFin  = [27,3,4,12,12,12,21,21,21,25,25,27,29,33, 3]
                ...
                print("remaped ", ndfArea['classe'].unique())
                # sys.exit()
                # get values uniques 
                ...
                nameexport = f"/globalTables/areaXclasse_CAATINGA_{nmodel}_vers_{vers}_ColS2.csv"
    ...
    lstColection = ['Col71', 'Col80', 'Col90']
    for col in lstColection:
        ...
        for pathLayerA in filesAreaCSV:
            ...
            if col in nameFiles and '_CAAT_' in nameFiles:     
                cc += 1    
                if 'remap' in  nameFiles:
                    nbacia = partes[-2].replace(".csv", "")
                else:
                    nbacia = partes[-2]
                print(f" ====== loading {nameFiles} ========") 
                dftmp = pd.read_csv(pathLayerA)
                ...
                dftmp["Colecao"] = col
                dftmp["Bacia"] = nbacia
                print(f" # {cc} >> {nbacia} ver tamanho {dftmp.shape}", )
                print(dftmp.head(2))
                if dftmp.shape[0] > 0:
                    lstDF.append(dftmp)
        # sys.exit()
        if len(lstDF) > 0:   
            ...
            lstInt = ['Colecao','year','classe','area']
            dfTest = ndfArea[lstInt].groupby(['Colecao','year','classe'], as_index= False).agg('sum')
            dfTest['Bacia'] = ['Caatinga'] * dfTest.shape[0]
            dfTest['Colecao'] = [col] * dfTest.shape[0]
            print(" 🫵 size dfTest ", dfTest.shape)
            print(dfTest.head(10))
            ...
            nameexport = f"/globalTables/areaXclasse_CAATINGA_{col}_red.csv"
