// https://code.earthengine.google.com/310239fdb1b7355dbd531876978c4db4
var param = { 
    ...
    // assetclass : 'projects/mapbiomas-workspace/AMOSTRAS/col9/CAATINGA/S2/POS-CLASS/toExport',
    // assetclass: 'projects/mapbiomas-workspace/AMOSTRAS/col9/CAATINGA/S2/POS-CLASS/ilumination', 
    assetclass: 'projects/mapbiomas-workspace/AMOSTRAS/col9/CAATINGA/S2/POS-CLASS/grass_Aflor', 
...
var lstBands = ['classification_2016',  'classification_2017',  'classification_2018',
...
function processoExportar(areaFeat, nameT){      
    var optExp = {
            ...
            'description': nameT, 
            'assetId': "projects/mapbiomas-workspace/AMOSTRAS/col9/CAATINGA/S2/POS-CLASS/" + nameT     
        };    
...
if (param.assetclass.indexOf('grass_Aflor') !== -1){
    mapS2_version = mapS2_version.filter(ee.Filter.eq('type_filter', 'grassland'));
}else if (param.assetclass.indexOf('Temporal') !== -1){
    mapS2_version = mapS2_version.filter(ee.Filter.eq('janela', cJanela));
}                       
print("show metadata fof maps versions coleção S2 ", mapS2_version);
print("know the size collection ", mapS2_version.size());

mapS2_version = mapS2_version.max();
...
// =========== exportando todos os poligons agrupados =========
var polygonsIlum = ee.FeatureCollection(region1_1)
                        .merge(ee.FeatureCollection(region2_1))
                        .merge(ee.FeatureCollection(region2_2))
                        .merge(ee.FeatureCollection(region3_1))
                        .merge(ee.FeatureCollection(region4_1))
                        .merge(ee.FeatureCollection(region5_1));
var nameExport = 'regions_iluminations_problems';

processoExportar(polygonsIlum, nameExport);