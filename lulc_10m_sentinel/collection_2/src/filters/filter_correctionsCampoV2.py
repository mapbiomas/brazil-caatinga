...
class processo_filter_campoAfloramento(object):
    ...
    def __init__(self, nameBacia,  modelo, nversion):
        ...
        if 'Temporal' in self.options['input_asset']:    
            self.imgClass = self.imgClass.filter(ee.Filter.eq('janela', 3)).first()
        else:
    ...
    def applyFilterInv(self):
        ...
        for yyear in tqdm(self.years):
            ...
            if str(yyear) == '2016':
                print(" corregindo ")
                maskGrassland16 = self.imgClass.select('classification_2016').eq(classGrass)
                maskGrassland17 = self.imgClass.select('classification_2017').eq(classGrass)
                
                masktoChange = maskGrassland16.subtract(maskGrassland17).gt(0) ## fica os pixels de campo que estão em 16 e não 17 
                rasterTemp = currentImage.where(masktoChange.eq(1), ee.Image.constant(4))   # sustituir por savana
            else:
                rasterTemp = currentImage
            if cc == 0:
                imgRasterbase = copy.deepcopy(rasterTemp.rename(bandActive))
            else:
                imgRasterbase = imgRasterbase.addBands(rasterTemp.rename(bandActive))       
    ...
    def processing_class_corrections(self, applyInverso):
        # apply the gap fill
        if applyInverso:
            imageFilled = self.applyFilterInv()
        else:
            imageFilled = self.applyFilter()

        print(" ----- filtro  aplicado -----")
        name_toexport = 'filterGA_BACIA_'+ str(self.id_bacias) + '_' +  self.model + "_GrV" + str(self.nversion)
        imageFilled = ee.Image(imageFilled).toByte().updateMask(self.raster_bacia).set(
                            'version', int(self.nversion), 
                            'biome', 'CAATINGA',
                            'source', 'geodatin',
                            'model', self.model,
                            'type_filter', 'grassland',
                            'collection', '2.0',
...
for idbacia in listaNameBacias[:]:    
    if idbacia not in listaNameB:
        ...
        aplicando_GrassAfloramento.processing_class_corrections(corregirY16)
