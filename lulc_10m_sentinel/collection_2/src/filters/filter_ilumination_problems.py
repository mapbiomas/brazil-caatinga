...
class processo_filter_Ilumination_problems(object):
    ...
    dictRegIlBacia = {
            ...
            '00000000000000000002': {
                'id_bacia': ["7584","76116","7612","761111"],
                'year': [2016, 2017,2018],
                'class_from': 21,
                'class_to': 4
            },            
    ...
    def __init__(self, nameBacia,  modelo, nversion):
        ...
        self.BaciatoCorr = False
        lstBaciasCoor = [kk for kk in self.dict_BaciaId.keys()]
        print("lista de bacias a corregir ", lstBaciasCoor)
        if nameBacia in lstBaciasCoor: #lstBaciasCoor
            self.BaciatoCorr = True
        ...
        self.regIlumProblems = ee.FeatureCollection(self.options['asset_regions_ilumination'])

        self.lstbandNames = ['classification_' + str(yy) for yy in range(self.options['year_inic'], self.options['year_end'] + 1)]
        self.years = [yy for yy in range(self.options['year_inic'], self.options['year_end'] + 1)]
        ...
        self.imgClassRef = (ee.ImageCollection(self.options['input_asset'])
                            .filter(ee.Filter.eq('id_bacia', nameBacia)) 
                                .filter(ee.Filter.eq('version', 1)).first()
                            )
    ...
    def applyFilter(self):
        ...
        lstImgMap = None
        previousImage = None        
        ###########  CORREGINDO DE 2016 PARA 2023 ############################
        # https://code.earthengine.google.com/63667dd0e4277dd2bb4abcb01ccb6f79
        lstIdReg = self.dict_BaciaId[self.id_bacias]
        ...
        for yyear in tqdm(self.years):
            ...
            rasterChange = currentImage
            print('raster band to change ', rasterChange.bandNames().getInfo())
            for idR in lstIdReg:
                dicttmp = self.dictRegIlBacia[idR]
                idsYears = dicttmp['year']                
                if int(yyear) in idsYears:
                    print(f" will change pixels in year {yyear} and polygon {idR}")
                    rasterPoss = self.imgClassRef.select(bandActive)
                    print("raster reference ===> ", rasterPoss.bandNames().getInfo())
                    polRecort = self.regIlumProblems.filter(ee.Filter.eq('system:index', idR)).geometry()
                    areaFixa = ee.Feature(self.geom_bacia.difference(polRecort), {'value': 0})
                    # print(areaFixa.getInfo())
                    # sys.exit()
                    areaToChange = ee.Feature(polRecort, {'value': 1})
                    areaTo_mask = ee.FeatureCollection([areaFixa, areaToChange])
                    img_mask_err = areaTo_mask.reduceToImage(['value'], ee.Reducer.first())
                    classDe = dicttmp['class_from']
                    classPara = dicttmp['class_to']
                    print(f"classe de {classDe} para classe {classPara}")
                    imgClassDe = rasterChange.updateMask(img_mask_err).eq(classDe).unmask(0)
                    imgClassPara = rasterPoss.updateMask(img_mask_err).eq(classPara).unmask(0)
                    rasterChange = rasterChange.where(imgClassDe.multiply(imgClassPara).eq(1), ee.Image.constant(classPara))

            if cc == 0:
                imgRasterbase = copy.deepcopy(rasterChange.rename(bandActive))
            else:
                imgRasterbase = imgRasterbase.addBands(rasterChange.rename(bandActive))       
    ...
    def processing_class_corrections(self):
        ...
        name_toexport = 'filterIL_BACIA_'+ str(self.id_bacias) + '_' +  self.model + "_V" + str(self.nversion)
        imageFilled = ee.Image(imageFilled).toByte().set(
                            'version', int(self.nversion), 
                            'biome', 'CAATINGA',
                            'source', 'geodatin',
                            'model', self.model,
                            'type_filter', 'ilumination',
                            'collection', '2.0',
...
# listaNameBacias = ['7622', '7616'
# #     '757','7712', '7721',  '7581', '7584', '7591', '7592', 
# # '76116', '7612', '761111', '7422', '752'
# ]
listaNameB = []
# addFlorest = False
models = "GTB"  # "RF", "GTB"
versionMap= 4
cont = 24
for idbacia in listaNameBacias[-12:]:    
    if idbacia not in listaNameB:
        print("-----------------------------------------")
        print("----- PROCESSING BACIA {} -------".format(idbacia))
        
        aplicando_correction_ilumination = processo_filter_Ilumination_problems(idbacia, models, versionMap) # added band connected is True
        aplicando_correction_ilumination.processing_class_corrections()
