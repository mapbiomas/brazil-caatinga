...
class ClassMosaic_indexs_Spectral(object):
    ...
    options = {
        ...
        'classMapB': [3, 4, 5, 9, 12, 13, 15, 18, 19, 20, 21, 22, 23, 24, 25, 26, 29, 30, 31, 32, 33, 36, 39, 40, 41, 46, 47, 48, 49, 50, 62, 75, 91],
        'classNew':  [3, 4, 3, 3, 12, 12, 15, 19, 19, 19, 21, 25, 25, 25, 25, 33, 29, 25, 33, 12, 33, 36, 19, 19, 19, 36, 36, 36,  4, 12, 19, 25, 25],
        ...
        'assetMapbiomas110': 'projects/mapbiomas-brazil/assets/LAND-COVER/COLLECTION-11/INTEGRATION/classification-ft',
        'asset_fire': 'projects/mapbiomas-public/assets/brazil/fire/collection5/mapbiomas_fire_collection5_annual_burned_v1',
        ...
        # FIXME: verificar caminho correto — ImageCollection com bandas burned_area_{year}
        # 'asset_mask_fire': 'projects/mapbiomas-workspace/AMOSTRAS/col11/CAATINGA/fire_masks',
        'asset_roads': 'projects/mapbiomas-workspace/AMOSTRAS/col11/ROADS/roads_federal_state_rasters_part2', 
        'asset_output': 'projects/mapbiomas-workspace/AMOSTRAS/col11/CAATINGA/S2/ROIs/ROIs_byBasinInd',
    ...
    def __init__(self, testando):
        ...
        # Carrega percentis pré-computados (compute_percentiles_sentinel.py)
        _perc_path = os.path.join(os.path.dirname(__file__), 'percentiles_sentinel_caatinga.json')
        if os.path.exists(_perc_path):
            with open(_perc_path, 'r') as _f:
                self.percentilesJson = json.load(_f)
            print(f"Percentis Sentinel carregados: {list(self.percentilesJson.keys())}")
        else:
            self.percentilesJson = None
            print("⚠️  percentiles_sentinel_caatinga.json não encontrado — sem normalização por percentil")

        print("see band Names the first ")


    def scale_mosaic_from_json(self, img, ano):
        """Stretch percentil p1/p99 nas bandas de reflectância → [0, 10000].
        Operação vetorizada: uma única subtração/divisão server-side, sem getInfo().
        Bandas sem percentil no JSON (slope, etc.) são mantidas via addBands(overwrite=True).
        """
        if self.percentilesJson is None:
            return img

        stats = self.percentilesJson.get(str(ano))
        if stats is None:
            print(f"⚠️  Sem percentis para ano {ano} — bandas não normalizadas")
            return img

        # Usa self.band_mosaic (Python list) para evitar getInfo() — sem chamada ao servidor
        bands_to_scale = [b for b in self.band_mosaic
                          if f"{b}_p1" in stats and f"{b}_p99" in stats]
        if not bands_to_scale:
            return img

        p_low  = ee.Image.constant([stats[f"{b}_p1"]  for b in bands_to_scale]).rename(bands_to_scale)
        p_high = ee.Image.constant([stats[f"{b}_p99"] for b in bands_to_scale]).rename(bands_to_scale)

        scaled = (img.select(bands_to_scale)
                    .subtract(p_low)
                    .divide(p_high.subtract(p_low).max(0.0001))
                    .clamp(0, 1)
                    .multiply(10000)
                    .toFloat())

        # addBands com overwrite=True substitui só as bandas escaladas; o resto fica intacto
    ...
    def agregateBandsIndexsPRI(self, img):        
        ...
        spriImgDry =   priImgDry.expression(
                            f"float((b('pri_median_dry{self.sufN}') + 1) / 2)").rename(['spri_median_dry']).toFloat()
    ...
    def agregateBandsContextoEstrutural(self, img):
        ...
        bandas_textura = [
            ...
            'ndfia', 'bsi_median', 'ui_median', 'awei_median'
        ]
    ...
    def CalculateIndice(self, imagem):
        ...
        imageW = self.agregateBandsContextoEstrutural(imageW)
    ...
    def get_mask_Fire_estatics_pixels(self, yyear, exportFire):
        janela = 5        
        imgColFire = ee.Image( self.options['asset_fire'])
        # print("image Fire imgColFire ", imgColFire.size().getInfo())
        intervalo_years = [f"burned_area_{kk}" for kk in self.lst_year if kk <= yyear and kk > yyear - janela]
    ...
    def iterate_GradesCaatinga(self, paridCod, lista_ids_gradeYY):
        ...
        # @collection110: mapas de uso e cobertura Mapbiomas ==> para extrair as areas estaveis
        lstBandMap = [
        ...
        collection110 = (ee.ImageCollection(self.options['assetMapbiomas110'])
                            .filter(ee.Filter.eq('version', self.options['col11_version']))
                            .mosaic()
                            .select(lstBandMap))
        ...
        # Estradas federais/estaduais: banda first_year_line (1985–2025)
        roads_byline = ee.ImageCollection(self.options['asset_roads']).mosaic()
        ...
        for anoCount in self.lst_year[:]:      
            ...
            if nomeBaciaEx in lista_ids_gradeYY:          
                ...
                # imMaskFireYY = imMaskFire.select(f"burned_area_{anoCount}").eq(0)
                # Erosão de 1 pixel (30m) nas bordas: pixel interior se e só se
                # todos os vizinhos têm o mesmo valor de classe
                _class_img = (collection110.select(bandActiva)
                                .remap(self.options['classMapB'], self.options['classNew'])
                                .rename([bandActiva]))
                _fmax = _class_img.focal_max(radius=1, kernelType='square', units='pixels')
                _fmin = _class_img.focal_min(radius=1, kernelType='square', units='pixels')
                map_yearAct = _class_img.updateMask(_fmax.eq(_fmin)).rename(['class'])
                # Sobrepor estradas como classe 25 (detectadas até o ano corrente)
                roads_yy = roads_byline.select('first_year_line').lte(anoCount)
                map_yearAct = map_yearAct.where(roads_yy, 25)
                if anoCount < 2025:
                    # loaded banda da coleção
                    imMaksAlertYY = imMaksAlert.select(bandActiva).eq(0)
                    imMaskSampleYY = imMaskSample.select(f"layer_samples_{anoCount}").eq(1) # removidos da segunda coleta "layer_samples_2024"
                else:
                    # 2025: sem máscara de alertas/amostras disponível na col10/col11
                    imMaksAlertYY = ee.Image.constant(1)
                    imMaskSampleYY = ee.Image.constant(1)


                # print("mascara imMaksAlert ", imMaksAlert.bandNames().getInfo()) .multiply(imMaskFireYY)
                areaColeta = imMaskSampleYY.multiply(imMaksAlertYY) 
                ...
                n_mosaic_imgs = img_recMosaic.size().getInfo()
                print(f" we loaded {n_mosaic_imgs} images ")

                if self.testando:
                    print(" ⚠️  quantity of images ", n_mosaic_imgs)
                ...
                img_recMosaic = self.scale_mosaic_from_json(img_recMosaic, anoCount)

                # Reprojetar camadas de 30m (col11 + máscaras) para alinhar com o grid Sentinel 10m
                proj_10m = img_recMosaic.select(0).projection().atScale(10)
                areaColeta = areaColeta.reproject(crs=proj_10m)
                map_yearAct = map_yearAct.reproject(crs=proj_10m)
    ...
    #                                                 ).filter(ee.Filter.inList('biome', self.options['biomas'])
    #                                                     ).filterBounds(gradeKM).select(self.options['featureBands'])        
...
def getPathCSV():
    ...
    mpath_bndImp = pathparent + '/dados/jsons/'
...
if searchFeatSaved: 
    ...
    print(f" ====== size das grade_year feitas {len(lstFeatAsset)} / {len(lstIdCode) * 10} faltando <> {len(lstIdCode) * 10 - len(lstFeatAsset)} ========= ")
    askingbySizeFC = False
else:
    lstFeatAsset = []
...
for cc, item in enumerate(lstIdCode[inicP: endP]):
    ...
    lst_item_yy = [f"gradeROIs_{item}_{myear}" for myear in range(2016, 2026) if f"gradeROIs_{item}_{myear}" not in lstFeatAsset]
    print(f'para a gride {item} >> we process >> {len(lst_item_yy)}')
