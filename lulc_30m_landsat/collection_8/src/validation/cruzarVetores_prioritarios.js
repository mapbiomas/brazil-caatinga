...
lst_nameAsset.forEach(function(nameAsset){
    ...
    if (nameAsset == 'UnidadesConservacao_S'){
        ...
        var lstUnid = ['Proteção Integral', 'Uso Sustentável', 'Proteção integral'];
        var shp_tmp_uso = shp_tmp.filter(ee.Filter.inList('TipoUso', lstUnid));
        var layerCovUCs = mapCol8.select('classification_2023').clip(shp_tmp_uso.geometry())
        var descExportLayer = 'uso_cobertura_2022_UCs'
        exportLayersofInterest(layerCovUCs.selfMask(), descExportLayer, limiteGeo)
    }
