...
def buildingPlots_Cruzando_Class(df_class, lstclass, nameArea):
    ...
    figPlotCr.update_yaxes(title='y', visible=False, showticklabels=False)
    figPlotCr.update_xaxes(title='x', visible=False, showticklabels=False)   
    # figPlot.show()
...
def plotUnicaBarra_extremosSerie(dfArea, nyear, nameArea, lstCClass):
    nameArea = nameArea.replace('areasPrioritCSV/', '')

    nwidth= 1600
    nheight= 1450
    dfAreaFtr = dfArea[dfArea['year'].isin([1985,2022])]
    dfAreaFtr['classe'] = dfAreaFtr['classe'].astype(str)
    print()
    # dfAreaFtr.sort_values(by=['cob_level1'])
    figBar = go.Figure()
    for cc, nclasse in enumerate(lstCClass):
        print(f"area classe {nclasse} \n {dfAreaFtr[dfAreaFtr['classe']== str(nclasse)]['area']}")
        figBar.add_trace(go.Bar(
                    x = dfAreaFtr[dfAreaFtr['classe']== str(nclasse)]['area'],
                    y = dfAreaFtr[dfAreaFtr['classe']== str(nclasse)]['year'],
                    name= dict_class[str(nclasse)],
                    orientation= 'h',
                    marker_color= dict_colors[dict_class[str(nclasse)]],
                    # text=[f'{kk}%' for kk in dfBiomas[dfBiomas['Biomas'] == nBioma]['percent'].tolist()]
                ))
         
    nwidth = 650
    nheight = 800
    figBar.update_layout(
        title_text='Áreas por Cobertura no Semiárido',
        title_font_size= 28,
        barmode='stack',
        height= nheight,
        width= nwidth,
        ...
        yaxis=dict(
            title="Área em Hectare",
            tickfont=dict(size=12),
            showticklabels=True
        ),
        font= dict(size=20),
        legend= dict(orientation="h", x= 0.20,  y=-0.30),
    ...
    # fig_1985.show()
    print("plot areas plot pie ", nameArea)
    namePlot = "graficosAreasPrior/{}_{}.png".format("BarraUnica_Areas_" + nameArea, str(nyear))
    figBar.write_image(namePlot, width= nwidth, height= nheight, scale=1)
    plt.clf()
...
for cc, nfile in enumerate(filesAreaCSV):
    ...
    if cc > -1:
        if 'sao_francisco' in nfile:
        ...
        elif 'UnidCons_S' in nfile:
            print("entroi in aqui klm")
            tmp_df_Area = pd.read_csv(nfile)
            tmp_df_Area = tmp_df_Area[(tmp_df_Area['classe'] != 0) & (tmp_df_Area['classe'] != 29)]
            tmp_df_Area = tmp_df_Area.drop(['system:index', '.geo'], axis='columns')
            tmp_df_Area = tmp_df_Area.apply(set_columncobertura, axis= 1)
            print(tmp_df_Area.head(2))
            lst_df_cons.append(tmp_df_Area)
            if len(lst_df_cons) > 1:
                groupUnidCons = True
            ...
            if savePlot:
                ...
                lstclass_part1 = [3,4,12,15,21,18,22,33]
                plotUnicaBarra_extremosSerie(df_Area, 2022, nameFileExp, lstclass_part1)                
                buildingPlots_Cruzando_Class(df_Area, lstclass_part1,  nameFileExp)
...
if groupUnidCons:
    print()
    ndfAcc = pd.concat(lst_df_cons, ignore_index= True)
    print("columna ", ndfAcc.columns)
    # print(ndfAcc.head(1))
    print("== AGRUPANDO PELA SUMA == ")
    groupDF = ndfAcc[lstAll].groupby(lstInt).agg('sum').reset_index();
    ...
    groupDF = groupDF.sort_values(by='year')
    print(groupDF.shape)
    calculoPerdas_ganho_byClass(groupDF, "areasPrioritCSV/unidade_ConservacaoAll")
    if savePlot:
        buildingPlots_x_Class(groupDF, classes,  "areasPrioritCSV/unidade_ConservacaoAll")    
        plotSunburstClasse(groupDF, 1985, "areasPrioritCSV/unidade_ConservacaoAll", dict_colors)
        plot2Pie_extremosSerie(groupDF, 1985, 'areasPrioritCSV/unidade_ConservacaoAll')
        plot2Pie_extremosSerie(groupDF, 2022, 'areasPrioritCSV/unidade_ConservacaoAll')

        buildingPlots_Cruzando_Class(groupDF, lstclass_part1,  "areasPrioritCSV/unidade_ConservacaoAll_p1_")
        buildingPlots_Cruzando_Class(groupDF, lstclass_part2,  "areasPrioritCSV/unidade_ConservacaoAll_p2_")
        lstclass_part1 = [3,4,12,15,21,18,22,33]
        plotUnicaBarra_extremosSerie(groupDF, 2022, "areasPrioritCSV/unidade_ConservacaoAll", lstclass_part1)