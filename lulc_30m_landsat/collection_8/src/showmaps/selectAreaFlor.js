...
var param = { 
    ...
    asset_filted : {
        'morfologico':'projects/mapbiomas-workspace/AMOSTRAS/col8/CAATINGA/POS-CLASS/morfologico'
    }, 
...
var imgMapCol8morfo =  ee.ImageCollection(param.asset_filted.morfologico)
                                    .filter(ee.Filter.eq('version', '1'))
                                    .filter(ee.Filter.eq('id_bacia', selBacia)).first();
imgMapCol8morfo = imgMapCol8morfo.select(banda_activa)//.remap(param.classMapB, param.classNew);
...
var mapDif = imgMapCol8morfo.eq(3).subtract(imgMapCol71temp.eq(3));

Map.addLayer(imgMapCol71temp, visualizar.visclassCC, 'Col71_' + String(yearcourrent), false);
Map.addLayer(imgMapCol8morfo.selfMask(),  visualizar.visclassCC, 'col8 morf', false);
Map.addLayer(mapDif.updateMask(mapDif.eq(0)),  {min: -1, max: 1, palette: '0000ff, ffffff, ff0000'}, 'mapDif');
