...
def processoExportar(mapaRF, nameB, ano, regionB):
    ...
    nomeDesc = 'RF_BACIA_'+ str(nameB) + '_'+ str(ano)
    
    ee.batch.Export.image.toAsset(image= mapaRF, description= nomeDesc, assetId=assetoutRF + nomeDesc, 
              pyramidingPolicy= {'.default': 'mode'},region= regionB, 
...
ROIg = ROIg.select(ee.List(dictPtos.keys()), ee.List(_dict.keys()))
...
for namB in nameBacias:
    baciaTemp = ee.Feature(baciasB.filterMetadata('ID_NIVEL2', 'equals', namB).first()).geometry().bounds().getInfo()
    
    for ano in anos:
        print('treinando ano %s da bacia %s' % (str(ano), namB))
        training = ROIg.filterMetadata('ano', 'equals', ano);
        ptosXbacia = training.filterBounds(baciaTemp)
        classifier = ee.Classifier.randomForest(numberOfTrees= 500, variablesPerSplit= 30, minLeafPopulation=1, outOfBagMode= True).train(ptosXbacia, 'class', bandNames);
        mosaicTemp = mosaicoTotal.filterMetadata('year', 'equals', ano).filterBounds(baciaTemp).mosaic()
        mosaicTemp = ee.Image(mosaicTemp).clip(baciaTemp)
        resl = 'RF_'+ str(namB) + '_' + str(ano)
        classified = mosaicTemp.classify(classifier, resl)
        try:
