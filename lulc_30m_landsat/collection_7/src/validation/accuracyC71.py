...
input_dir = os.getcwd()
output_dir = 'OUTPUT'
pointsAcc = "occTab_Caatinga_Col7_0.csv"
#version = 'v5'
...
def classification_report_shinny(df, level, class_names, class_values, year):
	...
	print("inlll djnddbn == ", user_acc)
	print("inlll djnddbn == ", user_se)
	print("inlll djnddbn == ", total_row)
	print(type(total_row))	

	if len(user_se) > total_row.shape[0]:
		mzeros = np.zeros(len(user_se))
		for ij in range(len(user_se)):
			if ij < total_row.shape[0]:
				mzeros[ij] = total_row[ij]
		total_row = mzeros
	print("inlll djnddbn == ", total_row)
	...
	# r_bacias = [nmBacia for kk in range(len(r_alloc_dis))]
...
def accuracy_assessment_all(df):
...
nameBacias = [
      '741','7421','7422','745','746','7492','752',
...
for nbacia in nameBacias:
	print('========== PROCESSIN BACIA ===== {} ========='.format(nbacia))
	df_tmp = dfprob[dfprob['bacia'] == int(nbacia)]
	# total_points = population_estimation(ndf)
	df_tmp = config_class_21(df_tmp)

	mdata = accuracy_assessment_all(df_tmp)

	for cc, file in enumerate(mdata):
		# print(file)
		text = ''
		for ff in file:
			text += str(ff) + ','
		...
		if 'ANO' in text:
			year = text[:-1].split(':')[1].replace(" ", "")

		elif 'area dis' in text:
			text = 'classe' + text + 'nameBacia,Year' 
			output_file.write(text + "\n")
		else:
			if len(text) > 35:
				text = text + nbacia +  ',' + year
				output_file.write(text + "\n")
